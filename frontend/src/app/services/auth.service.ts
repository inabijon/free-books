...
export class AuthService {
  ...
  Signup(user: any) {
    return this.http.post<{ result: any, message: string }>(BACKEND_URL + 'user/signup', user)
  }

  // Sign-in
  signIn(user: any) {
    return this.http
      .post<{ token: string, expiresIn: any }>(BACKEND_URL + 'user/login', user)
      .subscribe(
        (res) => {
          if (res.token) {
  ...
  getAllUsers() {
    return this.http.get(BACKEND_URL + 'user/users')
  }
